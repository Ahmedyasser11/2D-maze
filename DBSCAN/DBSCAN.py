...
def MyDBSCAN(D, eps, MinPts):
    ...
    for p in range(len(D)):
        if ret_labels[p]==0:
            sph_points = regionQuery(D,p,eps)
            if len(sph_points)<MinPts:
                ret_labels[p]=-1
            else:
                C+=1
                growCluster(D,ret_labels, p, sph_points,C, eps, MinPts)
...
def growCluster(D, labels, P, NeighborPts, C, eps, MinPts):
    ...
    for point in NeighborPts:
        if labels[point] ==0:
            nig_pint=regionQuery(D, point, eps)
            if len(nig_pint)>=MinPts:
                NeighborPts.extend(nig_pint)
            if labels[point]==0:
                labels[point]=C
    pass
...
def regionQuery(D, P, eps):
    ...
    for point in range(len(D)):
        result = ((((D[P][0] - D[point][0]) ** 2) + ((D[P][1] - D[point][1]) ** 2)) ** 0.5)
        if result<=eps:
            nig_points.append(point)
    return nig_points
